...
def checkForExistingTransactions(transaction, alreadyReadTransactions, newTransactionArray):
    if findExistingTransactionInArray(transaction, alreadyReadTransactions) == None:
        ...
        if duplicateNewTransaction:
            duplicateNewTransaction.transactionWords = updateTransactionWordsWithNewQuantity(
                duplicateNewTransaction, transaction.quantity)
            duplicateNewTransaction.quantity += transaction.quantity
        else:
            newTransactionArray.append(transaction)


def updateTransactionWordsWithNewQuantity(transaction, quantityToAdd):
    transactionWords = transaction.transactionWords
    wordSplit = transactionWords.split(" ")
    if len(wordSplit) == 12:
        wordSplit[3] = str(int(wordSplit[3]) + quantityToAdd)
        separator = " "
        return separator.join(wordSplit)
    elif len(wordSplit) == 10:
        wordSplit[1] = str(int(wordSplit[1]) + quantityToAdd)
        separator = " "
        return separator.join(wordSplit)
